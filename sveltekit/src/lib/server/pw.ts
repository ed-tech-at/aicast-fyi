...
export async function login(email: string, password: string): Promise<Response> {
  ...
  if (!user) {
    return json({ success: false, error: "Invalid email or password. Please try again." });
  }

  if (user.isDeleted) {
    return json({ success: false, error: "User does not exist." });
  }
  ...
  if (user.cryptVersion === 0) {
    ...
    return json({ success: false, error: "Unknown password encryption." });
  }

  if (!passwordMatch) {
    return json({ success: false, error: "Invalid login data. Please try again." });
  }
...
export async function register(email: string, password: string): Promise<Response> { /* | { success: false; error: string }*/
  ...
  if (existingUser) {
    ...
    if ('success' in loginResult && loginResult.success === false) {
      return json({
        success: false,
        error: "User already exists, but the password is incorrect."
      });
    }
